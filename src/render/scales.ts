...
export function drawRateScale(canvas: HTMLCanvasElement, o: RateScaleOptions): void {
  ...
  const baseline = crisp(9, dpr);
  const minorEnd = crisp(14, dpr);
  const majorEnd = crisp(20, dpr);
  ...
  ctx.fillStyle = o.ink;
  ctx.font = `${Math.round(9 * dpr)}px Archivo, system-ui, sans-serif`;
  ...
  for (const v of o.majors) {
    ...
    ctx.fillText(String(v), x, majorEnd + 3 * dpr);
  }

  // ── The two index marks, both above the baseline and concentric: DELIVERED is a
  //    hollow bone pointer, SET is a smaller filled amber pointer nested inside it.
  //    Coincidence reads as one mark with an amber core; any drift separates them
  //    visibly. That drift is the 20-25% error every other RSVP reader ships silently.
  const tip = crisp(0.5, dpr);
  const xd = crisp(at(o.delivered) / dpr, dpr);
  ctx.strokeStyle = o.ink;
  ctx.lineWidth = Math.max(1, Math.round(dpr));
  ctx.beginPath();
  ctx.moveTo(xd, baseline);
  ctx.lineTo(xd - 6 * dpr, tip);
  ctx.lineTo(xd + 6 * dpr, tip);
  ctx.closePath();
  ctx.stroke();
  ctx.lineWidth = 1;
  ...
  ctx.beginPath();
  ctx.moveTo(xs, baseline - 1 * dpr);
  ctx.lineTo(xs - 3 * dpr, tip + 3 * dpr);
  ctx.lineTo(xs + 3 * dpr, tip + 3 * dpr);
  ctx.closePath();
  ctx.fill();
